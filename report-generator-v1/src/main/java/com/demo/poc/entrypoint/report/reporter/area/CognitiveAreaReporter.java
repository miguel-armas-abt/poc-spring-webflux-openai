...
public class CognitiveAreaReporter implements AreaTypeReporter {
  ...
  public Mono<String> generateReport(Map<String, String> headers, FilePart image, ReportAggregate aggregate) {
    return jsonSerializer.readListFromFile("prompts/prompts.json", Prompt.class)
        .filter(prompt -> AreaType.COGNITIVE_AREA.getLabel().equals(prompt.getLabel()))
        .single()
        ...
        .map(response -> {
          ...
          aggregate.setCognitiveAreaReport(report);
          return report;
        });
  }
  ...
  public boolean supports(AreaType areaType) {
    return AreaType.COGNITIVE_AREA.equals(areaType);
  }
}
